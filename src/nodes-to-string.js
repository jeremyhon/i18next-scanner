...
const nodesToString = (nodes, options) => {
  ...
  const filteredNodes = ensureArray(nodes).filter((node) => {
    if (isJSXText(node)) {
  ...
  filteredNodes.forEach((node, nodeIndex) => {
    if (isJSXText(node)) {
      ...
      const attributeCount = ensureArray(
        node.openingElement?.attributes
      ).length;
      const filteredChildNodes = ensureArray(node.children).filter(
        (childNode) => {
      ...
      const shouldKeepChild =
        ...
        keepBasicHtmlNodesFor.indexOf(node.openingElement?.name?.name) > -1;
      ...
      if (selfClosing && shouldKeepChild && attributeCount === 0) {
        ...
        (childCount === 0 && !shouldKeepChild) ||
        (childCount === 0 && attributeCount !== 0)
      ) {
        ...
        attributeCount === 0 &&
        childCount === 1 &&
        (isJSXText(firstChildNode) ||
          isStringLiteral(firstChildNode?.expression))
      ) {
        // actual e.g. dolor <strong>bold</strong> amet
        // expected e.g. dolor <strong>bold</strong> amet
        memo += `<${nodeType}>${nodesToString(
          node.children,
          options
        )}</${nodeType}>`;
      } else {
